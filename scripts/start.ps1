...
try {
  ...
  if ($health.ok) {
    Open-Dashboard
    Write-Host "Stream Letters is already running. Dashboard opened."
    exit 0
  }
} catch {
...
if (-not (Test-Path (Join-Path $root "node_modules"))) {
  Write-Host "Installing Stream Letters dependencies..."
  & $npm install
  if ($LASTEXITCODE -ne 0) { throw "npm install failed." }
}

Write-Host "Building Stream Letters..."
& $npm run build
if ($LASTEXITCODE -ne 0) { throw "The Stream Letters build failed." }
...
for ($attempt = 0; $attempt -lt 120; $attempt++) {
  ...
  try {
    ...
    if ($health.ok) {
      Open-Dashboard
      Write-Host "Stream Letters is running. Dashboard opened."
      exit 0
    }
  } catch {
