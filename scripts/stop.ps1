...
if (-not (Test-Path -LiteralPath $pidFile)) {
  Write-Host "No Letter Chatter launcher process was found."
  exit 0
}
...
if (-not $process) {
  ...
  Write-Host "Letter Chatter was already stopped."
  exit 0
}
...
if ($process.ProcessName -ne "node" -or $details.CommandLine -notlike "*dist*src*server.js*") {
  throw "PID $serverPid does not look like the Letter Chatter server; it was not stopped."
}

Stop-Process -Id $serverPid
Remove-Item -LiteralPath $pidFile -Force
Write-Host "Letter Chatter stopped."
